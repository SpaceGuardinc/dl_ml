...
def training(epoch):
    ...
    N = 10000 #число итераций при обучение
    count = len(epoch)
    for k in range(N):
        x = epoch[np.random.randint(0, count)] #случайный выбор входного сигнала из обучающей выборки
        y, out = go_forward(x[0:3]) #прямой проход по Нейронной Сети и вычесления входных значений нейронной сети
        e = y - x[-1] # ошибка
...
training(epoch) # запуск обучения

#проверка полученных результатов
for x in epoch:
    y, out = go_forward(x[0:3])
    print(f"Выходные значения Нейронной сети: {y} => {x[-1]}")
